...
class iplb (object):
  ...
  def __init__ (self, connection, service_ip, servers = []):
    ...
    self.numOfSessions = {"10.10.1.100":0, "10.10.1.200":0} #IP -> number of sessions
  ...
  def _handle_PacketIn (self, event):
    ...
    if ipp.srcip in self.servers:
      ...
      if entry is None or entry.server not in self.live_servers:
        ...
        totalSessions=self.numOfSessions["10.10.1.100"] + self.numOfSessions["10.10.1.200"]
        log.info("-------------------------------------------------------------------------------------")
        log.info("---%s------Sessions----", totalSessions)
        log.info("--------------------------------------------------------------------------------------")
        if(totalSessions%3==0):
          server = "10.10.1.100"
